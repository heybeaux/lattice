...
/** Maximum number of concurrent pipeline spans tracked at once. */
const PIPELINE_SPAN_MAX_SIZE = 1_000;

/**
 * Maximum age of a pipeline span before it is considered stale and eligible
 * for eviction (10 minutes). Covers the case where a pipeline crashes before
 * emitting `pipeline:completed` or `pipeline:aborted`.
 */
const PIPELINE_SPAN_TTL_MS = 10 * 60 * 1_000;
...
export class OtelExporter {
   ...
   *
   * Bounded by {@link PIPELINE_SPAN_MAX_SIZE} and a {@link PIPELINE_SPAN_TTL_MS}
   * TTL so stale entries from crashed pipelines do not accumulate indefinitely.
   */
  ...
  private _onPipelineStarted(event: LatticeEvent): void {
    ...
    // Evict stale / excess entries before inserting so the map stays bounded.
    this._evictStalePipelineSpans();
  ...
  /**
   * Remove pipeline spans that have exceeded {@link PIPELINE_SPAN_TTL_MS} or
   * that push the map over {@link PIPELINE_SPAN_MAX_SIZE}. Called before each
   * insert so the map stays bounded even when pipelines crash without emitting
   * their terminal event.
   */
  private _evictStalePipelineSpans(): void {
    const now = Date.now();

    // First pass: remove TTL-expired entries.
    for (const [key, span] of this.activePipelineSpans) {
      if (now - span.startTime > PIPELINE_SPAN_TTL_MS) {
        this.activePipelineSpans.delete(key);
      }
    }

    // Second pass: if still over the cap, remove oldest entries (insertion order).
    if (this.activePipelineSpans.size >= PIPELINE_SPAN_MAX_SIZE) {
      const excess = this.activePipelineSpans.size - PIPELINE_SPAN_MAX_SIZE + 1;
      let removed = 0;
      for (const key of this.activePipelineSpans.keys()) {
        this.activePipelineSpans.delete(key);
  ...
  private _startRealOtelSpan(span: TrackedSpan): unknown {
    ...
    return tracer.startSpan(span.name, opts, opts['_parentContext'] as unknown);
  }
